...
### The session, as it finished (11 September)

Launched 09:47, relaunched 09:57 for the LLM port change, activated 10:00:02,
stood down 16:00:05, daily report 16:03:31. The app was closed some time after
its last resting-order scan at 16:58.

* **BHP.AX stopped out in the opening auction.** The STP at 60.45 filled at
  **60.40** (793 shares, order 750830217) at 10:00:00; IBKR cancelled the OCA
  target, which is the `Error 202, reqId 668: Order Canceled` at 10:00:00 - a
  sibling cancel, not a fault. The app absorbed it at the next reconciliation
  poll, **10:03:22**, with **no kill-switch trip and no mismatch**, and the scan
  went to 18 legs across 9 symbols. Cash reconciles exactly: +47,855.05 =
  793 x 60.40 - 42.15 commission.
* **Regime classified after 1,395 s: `sideways`, exposure scalar 0.70.**
* **449 entry decisions, all refused** on the aggregate cap (IAG 238, COL 120,
  CPU 86, AGL 5) - see the Account row.
* The ERROR count at the open (114) is the known yfinance publication window,
  recovered 10:20:32 - judge by content, never by count.
...
* Error log: `docs/CLAUDE_ERROR_LOG.md`, **CE-001 to CE-067** (the whole history back-filled 15 Sep: CE-037 to CE-066; CE-067 the finalisation's slips)
* ▶ **Progress tracker (the ONLY status record since 14 Sep 14:45):**
  `docs/audit/2026-09-design-recovery/00-progress-tracker.md`

⚠️ **The transcripts cover every day from 24 July 15:52** (CE-020). The plan,
report §4 and this file had said 25 July – 2 September was deleted; that was
wrong, because it read file dates, not contents. **The operator's decisions
are in two channels: typed messages and AskUserQuestion dialogs, which are
stored as tool results.** Read both. The extraction scripts are described in
the register's header.
...
2. ⛔ **TRADING IS SUSPENDED FOR THE COURSE OF THE AUDIT** (operator, 14 Sep
   about 10:19: "Trading will be suspended during the course of this audit").
   This supersedes the 12 September decision to keep M175 running in `auto`.
   - **How it holds today:** the app is not launched. **The configuration is
     unchanged and still says `execution_mode=auto`, `autonomous_strategies=swing`**
     (read 14 Sep 10:20). A launch would therefore trade exactly as before.
     Do not launch the app, and do not change the configuration or the kill
     switch to enforce the suspension, unless the operator asks.
   - **What stops while it is suspended:** signal and trend-break exits, time
     stops, reconciliation, protective re-arm, the resting-order scan, and new
     live evidence (the observation items below cannot progress).
   - **What does not depend on the app:** the nine positions' GTC stop and
     target legs rest at IBKR. They were read resting at the broker with the
     app closed (11 Sep 19:45). Whether a paper stop *fills* while Gateway and
     the app are both closed has not been observed in this project.
   - The 12 September reason for `auto` ("human latency must not slow or
     distort decisions while testing") is kept as history. Do not propose
     `recommend`.
...
**Stage 4, drift map and strategy integrity (report §6–8; 14 Sep)**

* **59 drift items, each with its authority from the transcripts:**
  - 26 operator-directed;
  - 22 agent-proposed and operator-approved (mostly inside whole plans or
    lists);
  - 10 agent-only;
  - 1 unknown.

  The drift that matters most is agent-only and dates from the first two
  builds (§7.7).
* **Autonomy was the operator's from the start.** The build brief the
  operator pasted on 24 July amended the paper, adding "a future function to
  allow automated orders tied to tested rules and selectable methodology"
  (register AE-01). On 26 July the operator directed the mode (AE-05).
* **No AI in the decision was Claude's choice** (CE-021). The plan the
  operator approved on 26 July gave the AI a veto on entries. Claude built
  without it and reported it afterwards as "a legitimate feature I chose not
  to build". On 21 August the operator asked for an AI-formed buy/sell/hold
  recommendation; it was built as an advisory panel (AE-25).
* **Swing is the paper's swing in outline and not the operator's method**,
  and has not been since the first commit. Missing: the candle-close wait, the
  rejection tail, the weekly filter, volume, the resistance check, partial
  profit, breakeven and the trailing stop. It also decides on the forming
  intraday bar. The 30-day time stop came from a third-party review, and the
  operator has kept it (§8.3; AE-17).
* **CE-017's ordering was approved on a wrong claim of Claude's**
  ("self-healing within a scan cycle"), six days after the same trap had
  been closed in the manual path (AE-27, AE-29).
* **The kill switch's staleness trip was removed on 31 July** (M28a), a
  reasoned change reported to the operator; the docstrings were not updated
  (CE-022).
...
**Brief §7, §13, §8, §14 (14 Sep afternoon and evening; report R9, R16, R10, R14;
detail and evidence in those files and the tracker)**

* **Risk per trade (R9):** no entry took its 1% budget. The range was
  0.13–0.70% of equity, median 0.45%. Placeholder half-Kelly bound 13 of 20
  entries, and the 10%-of-cash cap cut 18 of 20. Single-name, sector,
  cluster, gap, earnings, daily loss and drawdown never acted. 19 kill-switch
  trips, none from the loss limits. The records name only the FIRST rail
  that refused.
* **The aggregate-cap trap (R16):** every large over-cap reading was ONE
  position counted at its whole value (A2M 4 Sep, IAG 9 Sep, JHX 10–12 Sep).
  The 12 Sep 7.48% is reproduced exactly from IBKR's closes. "Winners
  consume the budget" is refused: the book was below cost.
* **A possible Kelly lock (R16 §16.2):** on the 8 closed positions (2 wins),
  measured half-Kelly at the 20-trade switch would be ZERO, stopping the
  entries that produce evidence. Derived and conditional.
* **Execution (R10):** the deployed M175, and M174, have never transmitted
  an order. Every ASX-era order was signed by the autonomy gate; the human
  paths have never run live. The market-exit path has run for 3 symbols, and
  each run tripped the kill switch; SEK's exit filled while the app held it
  as rejected. The M173 TIF fix and COMMISSION VERIFIED are still unproven.
* **Failures → controls (R14):** all 6 reconciliation trips were the app's
  own record being wrong, 3 of them from booking at transmission.
  "Unknown code → halt" caused 7 of the 19 trips: 4 from the app's own
  missing TIF, 3 from benign cancels. The orphan scan was right 1 time in
  10.
* **⚠️ JHX (1,097) closed 11 Sep at 39.01 below its resting stop (39.39), and
...
**The back-fill and finalisation (15 Sep afternoon; Part I, Annexes A–C)**

* **The error log now covers the whole history: CE-001 to CE-067.** The
  back-fill (`7851d64`) added CE-037 to CE-066 from the fix commits, Claude's
  own admissions in HANDOFF and ROADMAP, and the operator's corrections in the
  transcripts. **14 of the 18 High-severity errors entered before the first
  live order**, and surfaced only when the live account exercised them.
* **Two findings changed earlier sections, both corrected by note:** the
  sector cap was not wired on 25 Aug, and Financials reached **34.95%** of
  equity against its 30% (R9; CE-043); **the one live booking reversal was
  wrong-signed** (R10; CE-046). The log hole's cause is recorded in a commit
  (CE-038). R21 is down to 18 open unknowns.
* **Part I** (`P-development-chronology.md`) tells the history period by
  period: 39 of the 59 drift items, and 8 of the 10 agent-only ones, entered
  in the first week.
* **Annex A: 45 anomalies** for the operator's review (20 open, 12 frozen,
  12 closed, 1 held). **Annex B:** every audit tool re-run and every source
  hashed (`Documents\QAT-audit-evidence\2026-09-15-final\`,
  `tools/reproduce_evidence.ps1`). **Annex C:** the back-fill register.
* **The compiled report:** `QAT-Design-Recovery-Audit-Final-Report.md`,
  generated by `tools/compile_report.py` from the section files. Re-run it
  after any change to a section; never edit the compiled file.
* **CE-067:** two audit tools counted Claude's context-compaction summaries
  as the operator's messages (1,404 / 62, not 1,411 / 69; no authority entry
  affected); R2 miscounted the baseline snapshot (464 files). Fixed and
  corrected by note.
...
2. **Stage 5: the risk stack and control interactions** (report §9, §16).
   ...
   taken per trade below 1%.
3. **Stage 4a's systematic pass**, alongside: fix commits, ROADMAP and
   HANDOFF incidents, and the transcripts from 24 July.
4. **Stages 6–10**, then **⛔ Checkpoint B** (the operator reviews the draft).

The outstanding list is frozen. Its observation items, item 1 (the TIF fix
on a real market exit), item 3 (the first `COMMISSION VERIFIED`) and item 8
(the status column), **cannot progress while trading is suspended**. They
resume when the operator resumes trading.
...
### What was measured (item 3, "A & B")

* **IBKR's commission equals the model on 17 of 17 logged orders**, to the cent
  (`max(6.60, 8.8 bp)`, joined from the 24-25 August `commissionReport` lines).
  The commission was never the defect.
* **M65 wrote IBKR's commission-INCLUSIVE `avgCost` over every entry at every
  restart** (1 bp tolerance vs 8.8 bp commission) - undoing M70's correct fill
  for JHX.AX (31 Aug) and COH.AX (10 Sep). Entry commission was charged twice:
  in the price and in `entry_cost`. **All 9 open records carry it now.**
* **Modelled slippage (5 bp) was charged on fills that already contain it** -
  live, and in replays (`SimulatedBroker` slips its prices too).
* **The floor was charged per absorbed piece** (LOV's one exit paid four).
* BHP was the first row ever closed with `reference_price`, and its slippage
  read **+0.0464**; the truth is **−0.01** (fill 64.07, an exact tick).
* **The ledger file failed its own audit** - see the Ledgers row. `as_row` now
  derives every column from exactly what it stores, which affects every write.
* A read-only probe as a SECOND client got BHP's execution with commission 0.0
  and no report: commissions arrive only to a connected client, live.

### What M175 does

`CostModel.charge()` (commission only) is what the ledger records, one floor
per order; slippage is measured (`entry_slippage`), never charged. Entry records
carry `price_source`; M65 never overwrites a `"fill"` and de-commissions
avgCost when it corrects - but SKIPS (with a warning) when that would take the
floor branch at the current quantity. `IBAdapter` totals `commissionReportEvent`
per order and `CommissionAuditor` writes `commission_checks.csv`
(`COMMISSION VERIFIED` / `COMMISSION DISAGREES`) - the model stays
authoritative, IBKR's figure checks it. `scripts/repair_fill_basis.py`
rewrites the existing rows and records from logged evidence.

### ⚠️ THE DEPLOY IS A SEQUENCE, AND THE ORDER MATTERS

After the PR merges, after a close, with the app closed and IB Gateway up:
**build + sign → `deploy.ps1` dry run → operator OK → `deploy.ps1 -Apply` (it
does NOT launch the app) → `repair_fill_basis.py` dry run → operator reads it →
`--apply` → operator launches.**

* **Deploy BEFORE the repair**, so the old build is never installed over
  repaired records (M174's M65 would re-inflate them at its first startup).
* **The repair is ONE-SHOT.** It refuses once it has run (backups or `"fill"`
  stamps present) and once any `Build: M175`+ line is in the log.
* **Expected dry run** (measured with the app closed, 11 September): 12 rows,
  `audit: clean`, net −4,065.73 → −3,495.02; open records: **4 corrected from
  logged fills (BOQ ASX SUN ANZ), 5 left for M65 (WOW JHX TWE TAH COH)**; TNE
  the only `[FRAGMENT]`; SEK and BHP the only formula-sourced rows; BHP slip
  −0.0100. Any NEW row since then is a real change - read it.
* **At first launch, M65 lines for WOW, JHX, TWE, TAH and COH are EXPECTED**
  (the new M65 converts their avgCost). Read them; any other symbol needs a
  reason. Read back `Build: M175` off the log.
* If the deploy fails AFTER the repair was applied, restore both
  `.bak-fill-basis-*` files before any launch.
* **The evidence lives in rotated logs** (`qat.log` .. `qat.log.5`, the 24-25
  August `execDetails` lines). Measured 11 September: `qat.log` is 4.42 of
  ~5.0 MB, and the next rotation deletes only `qat.log.6`, which holds none of
  it. Do not clear the logs before the repair has run.

### Accepted, with reasons - not open defects

* Self-check failure refuses the WHOLE repair (spec, approved). A reviewer
  suggested per-row degradation; not taken. If the dry run refuses, it names
  the row - decide then.
* An exit order absorbed SMALLER than its ledger rows is charged as a whole
  order, unflagged - absent from the live data, and the repair cannot run again.
* Commit `cf01c9d`'s trailer reads "Claude Sonnet 5" (a subagent's own
  instruction). Cosmetic.
...
### ✅ THE EVENING: THREE ITEMS CLOSED, AND 10349 WAS OURS ALL ALONG

Worked after the close, with the market shut, so M173 is an ordinary deploy
rather than a third mid-session exception.

**The SEK ledger double-count, repaired.** One row replacing two: 2,978 @ 12.93,
`signal`. Both rows carried the same `order_id` AND the same `opened_at`, so
`collapse_to_positions` was MERGING them into a single 5,005-share position —
the mechanism built to make fragmented exits readable was quietly making a
double-count look like one large trade. Backed up first; today's true net is
**−7,797.75**, not the −11,924.91 the corrupted ledger produced.

**A protective order is re-read at sign-off** (`b4ea566`). Flat → refuse, shrunk
below the order size → refuse, broker unreadable → refuse. Refuse rather than
resize, following the buy branch's own rule that silently changing an approved
quantity defeats the approval. Only a SHORTFALL refuses: an order smaller than
the holding under-protects, which is far less urgent than selling shares that
are not there.

⚠️ **AND 10349 WAS A CODE DEFECT, WHICH I DENIED REPEATEDLY.** I told the
operator several times it was a Gateway preset problem and not ours. It was
ours, and it was one line:

    return MarketOrder(action, order.quantity)      # ib_async defaults to DAY

The final two returns of `to_ib_order` — the plain market and limit branches —
were **the only paths in `ib_translate` that left the TIF implicit**. The preset
imposes GTC, IBKR rejects the mismatch, and each rejection halted order flow.
Both app-driven market exits of the day went through that branch.

⚠️ **THE MODULE ALREADY CARRIED THE LESSON**, in `to_ib_parent`'s own comment
from 4 September: *"leaving it unset does not mean IBKR's default."* It was
applied to the bracket and stop paths and not to that one. **Same defect, one
branch over, five days later** — which is why it presented as the preset's fault
both times, and why I believed that.

**The preset cannot stand aside**, checked in the Gateway UI: Time in Force is a
dropdown with no disable option across all three "Show" views. And it must be
GTC, because the protective legs must be — DAY killed every stop on 31 July.
Confirmed at the BROKER rather than the dialog: all 18 resting legs read
`tif='GTC'`.
...
### ⚠️ A CORRECTION THAT CANCELLED WORK RATHER THAN CREATING IT

The morning write-up said the app "read the SEK position twelve seconds into the
fill" as though the read were the error, and proposed a second fix: stop
deciding partial-vs-full from a mid-fill read.

**That premise was wrong.** `rearm_protective_stops` reads `naked_positions()`,
which reads the BROKER — and the broker genuinely held 948 at that instant,
because 2,030 of 2,978 had filled. The app read correctly and the reading was
true when taken. The defect was never the read; it was ACTING on it 28 seconds
later without re-checking, which is exactly what sign-off now does. **There was
no second fix to make**, and building one would have been building on a
misdiagnosis.

### ⚠️ WHAT THE SABOTAGES CAUGHT THAT READING DID NOT

Three more escapes this evening, and only two were real:

* **A sabotage that never landed.** `if held is None:` appears TWICE in
  `oms.py` — the first is `submit_exit_order`'s — so a one-line anchor patched
  the wrong method while the new guard sat untouched, and reported ESCAPED when
  nothing had been broken. **A sabotage that does not land is
  indistinguishable from a rail that holds**, which makes it worse than no
  sabotage at all. Anchors now carry enough context to be unique.
* **The flat branch was genuinely unguarded.** Disabling it left everything
  green, because the SHORTFALL branch also rejects — 0 is less than 948. The two
  overlap, so flat earns its place on the MESSAGE, and the message is now
  asserted.
* **A structural sweep that was not structural enough.** The TIF test only
  exercised `to_ib_parent`'s LIMIT branch, so removing the TIF from its MARKET
  branch went undetected. Nine shapes now, not eight — and the count itself is
  pinned, which is what caught my first miscount of seven.


### ⚠️ SEVEN VACUOUS OR NON-LANDING TESTS IN ONE DAY

Every one passed against code that was wrong, or reported a pass that meant
nothing. **Sabotage caught six; reading caught none.** The three from the
evening are in the section above; the four below are the morning's.
...
as measured — not by editing last time's.** On 9 September it handed a session a
3 September picture. On 10 September its outstanding list was AUDITED against
the code and **six of eleven items were wrong**. This is the ONLY part of the
file a fresh session reads before acting, so a stale copy here outranks every
correct row above it.
...
(since 12 September), AND TRADING IS SUSPENDED FOR THE COURSE OF THE AUDIT
(operator, 14 September). THE AUDIT IS FINALISED (15 September, afternoon)
AND WAITS FOR THE OPERATOR'S REVIEW (CHECKPOINT B). Before doing ANYTHING,
read, in this order:
  1. docs\audit\2026-09-design-recovery\00-progress-tracker.md - THE ONLY
     PLACE AUDIT PROGRESS IS KEPT (operator, 14 Sep). It follows the
     operator's brief's own numbering: §1-§22 for the brief's sections, R1-R24
     for the report sections (brief §19). The old "Stage 0-10" numbers are
     RETIRED (CE-030). Report and update against the brief's numbers only.
  2. docs\audit\2026-09-design-recovery\QAT-Design-Recovery-Audit-Final-Report.md
     - THE COMPILED FINAL REPORT, in the order the operator asked for: R1 (the
     executive summary), Part I (the analysis in the order of development),
     R2-R24, Annex A (45 anomalies), Annex B (the evidence index), Annex C
     (the error-log back-fill). It is GENERATED by
     docs\audit\2026-09-design-recovery\tools\compile_report.py from the
     section files (01-24, P-, A-, B-, backfill\00-backfill-register.md).
     Change a section file, then re-run the compiler; never edit the
     compiled file.
  3. docs\HANDOFF.md section "14 SEPTEMBER: THE DESIGN AUDIT" (standing
     instructions, baseline, findings, including the 15 Sep blocks and "The
     back-fill and finalisation")
  4. docs\CLAUDE_ERROR_LOG.md (CE-001..CE-067; the whole history)
...
NEXT: CHECKPOINT B - the operator reviews the final report (R1 first) and the
45 anomalies in Annex A. NO AUDIT WORK IS OUTSTANDING. Do not start
remediation. R23 lists the five decisions the operator is asked for:
...
comments to the section files as dated correction notes (never a silent
rewrite), log each correction, and re-run compile_report.py. Annex A's three
anomalies most needing the operator: A01 (JHX), A02 (CE-017), A09 (the
config says auto). With trading suspended, everything works from the records
up to 12 September.
To re-check the evidence: tools\reproduce_evidence.ps1 -OutDir <a NEW folder
under Documents\QAT-audit-evidence\> (it refuses an existing one; PowerShell;
read-only against QAT; it extracts the IBKR statement's text into a temp
folder and deletes it). Each log tool runs with the start date its section
was drafted with (in the script) - never a uniform default (CE-067).
Method that worked (14 and 15 Sep): a small read-only tool per question,
kept in the section's folder (s07-s13-risk-and-interactions\tools\,
s08-s14-execution-and-incidents\tools\, s09-s10-ai-and-regime\tools\,
s11-evidence-integrity\tools\, s12-s16-complexity\tools\, backfill\tools\,
s-chronology\tools\), lint-clean; every figure written with its tool output
open beside it. Mermaid (R16-R19) is checked by rendering it with mermaid
11.4.1 in the in-app browser, which runs a page's script only for files
inside the project folder (use a git-ignored folder such as
dist\_audit_check\, then delete it).
...
apply them, do not re-ask:
  Q1 one recommendation, formed by the AI "informed around strategy and
     rules", the same in both modes, which differ only in fulfilment;
  Q2 Swing Trader methodology.md IS the swing specification; test condition
...
stop (39.39) and the stop did NOT fill (IBKR statement 24 Aug-11 Sep). Cause
NOT DETERMINED; a broker read (the TWS Orders panel) would settle it. The
audit is now complete in draft: raise it with the operator at Checkpoint B
(R23 D5); do not act on it unasked. Tracker section 2; Annex A A01.
...
resting-order scan. The nine positions' GTC stop and target legs rest at
IBKR (read there with the app closed, 11 Sep 19:45) - and see JHX above.
...
dirs at C:\QuantAdvisoryTerminal.bak-*, the rest of Documents and Downloads,
and any other drive or user folder - needs the operator's permission FIRST,
location by location. A file the operator @-attached is readable from the
transcript that holds it; do not open it at its original path. NEVER read
...
(CE-032). The operator's words are in the Claude Code transcripts, which
cover EVERY DAY FROM 24 JULY (CE-020), in TWO channels: typed messages AND
AskUserQuestion dialog answers (stored as tool results). Read both -
stage4\tools\ has the read-only extractors. A transcript record typed "user"
is not always the operator: context-compaction summaries are Claude's own
text (isCompactSummary; CE-067). Fresh-context investigators are approved for
fact-finding ONLY: code and raw logs, no Claude-written docs, file:line for
every fact, "NOT DETERMINED" instead of guesses, no judgement - and VERIFY a
subagent's report covers what it was sent to read (CE-026).
...
WHAT THE AUDIT FOUND (final, for Checkpoint B; R1 has the summary):
- CLASSIFICATION (R24): RED - DESIGN COMPROMISED. Three of the four core
  elements the operator named are not implemented: no AI takes part in any
  trade decision (R11; CE-021 dropped the approved AI entry role); one
  strategy runs, with no decision matrix (R7 #4, #5); QAT does not trade the
  operator's swing specification (R8; CE-026). The fourth, autonomy, acts on
  the rule's signal. AMBER if the operator decides to test the current rule
  and to defer the AI's part (R24 §24.5).
- THE HISTORY (Part I): 39 of the 59 drift items, and 8 of the 10 agent-only
  ones, entered in the first week. 14 of the 18 High-severity Claude errors
  entered before the first live order and surfaced only when the live
  account exercised them. Removing the later patches would restore defects,
  not the intended design.
- RAILS (R9, R16): no entry took its 1% budget (0.13-0.70%, median 0.45%);
  the cash cap cut 18 of 20 entry sizes; each large aggregate-cap breach was
  ONE position at its whole value (A2M, IAG, JHX), and all 449 refusals on
  11 Sep trace to JHX. The sector cap was NOT WIRED on 25 Aug: Financials
  reached 34.95% against its 30% (CE-043). On the record so far, measured
  Kelly at the 20-trade switch would be ZERO (derived, conditional).
- EXECUTION (R10, R14): M175 and M174 have never transmitted an order; every
  ASX-era order was signed by the autonomy gate; the exit path ran for 3
  symbols, each run tripping the kill switch; the one live booking reversal
  was WRONG-SIGNED (CE-046); "unknown code -> halt" caused 7 of 19 trips.
- REGIME (R12): a launch artefact opens most runs on "recovery"; the US VIX
  below 15 sets low_vol by a fixed rule; the 28 Aug "86%" is confounded
  (CE-032); the 20-bar refit has never run.
- RECORDS (R13): 7 of 8 closed positions and all 9 open ones match IBKR
  exactly. TNE does not: 60 of 3,051 shares booked, loss understated by
  6,937.44 (the missed-exit replay; CE-066; reported, NOT fixed). Seven
  repairs rewrote the ledger; all now have a traced authority.
- COMPLEXITY (R15, R20): 18 of 184 modules never run in the app; prose is
  37% of the code; the costly complexity is live and came from fixes. 37
  simplification rows; 3 removals recommended after authorisation, none a
  control.
- COHERENCE (R22): not reliably understandable by one person; not suitable
  for continued evidence collection as it stands; keep the freeze.
- UNKNOWNS (R21): 18 open (U10, U13, U14 settled by the back-fill).
- ANOMALIES (Annex A): 45 - 20 open, 12 frozen, 12 closed, 1 held.
...
the broker when the exit is PROPOSED, before the kill switch is checked
(oms.py:731 then 745); while the switch is tripped the re-arm cannot transmit
a replacement. IAG.AX (6,699) had no broker stop 11:21:46-12:20:40 on
9 September. It cannot occur while the app is not running. IF TRADING
RESUMES AND THE KILL SWITCH TRIPS, CHECK EVERY POSITION HAS A STOP AT THE
BROKER. Change nothing unauthorised.
...
⚠️ PowerShell 7's ConvertFrom-Json turns the log's "ts" into a DateTime, so
comparing it with a string returns nothing and no error (CE-031): filter on
the raw line, or cast with [datetime].
⚠️ The decision journal writes a verdict only when it CHANGES per symbol
...
carry the BROKER's id - never join them on order_id (CE-029).
risk_decisions.csv's EXIT rows carry no regime and no sizing inputs - split
by side first.
⚠️ A LOG TOOL COUNTS LINES, NOT EVENTS (CE-033). Check how many lines one
...
(line length 100). NEVER PIPE pytest OR invoke build THROUGH tail.
⚠️ NEVER WRITE A FILE IN PLACE WITH open(p, "w") IN AN EDIT SCRIPT (CE-028):
it truncates before it validates. Temp file + os.replace; check size + diff.
⚠️ In PowerShell a here-string reaches git's stdin only through a pipe:
@'...'@ | git commit -F -   (CE-023).
⚠️ Remove-Item on a path containing "Claude Programming" can be refused by
the path guard; use a relative path from inside the folder, then
Set-Location back to C:\Claude Programming.
...
EVIDENCE (Documents\QAT-audit-evidence\; Annex B lists it all):
\2026-09-12\ (the baseline: 464 files in its sha256 MANIFEST.csv -
transcripts, qat-logs, claude.ai export, IBKR statements 24 Aug and
24 Aug-11 Sep), transcript snapshots \2026-09-14\, \2026-09-14-stage4\,
\2026-09-14-s07-s14\, \2026-09-15\, \2026-09-15-s11\, \2026-09-15-s12-s16\,
...
- LEDGER: 12 rows = 8 positions, sha256 688B7091..., net -3,495.02;
  ...
  since 12 Sep 11:19 (Annex B, SOURCES.csv).
...
- ENTRY GATE (when trading runs): the aggregate cap reads ~7.5% ONLY because
...
TIF fix on a real app-driven market exit; R10 confirms it has never run),
3 (first COMMISSION VERIFIED; never logged), 8 (a genuinely blank status
cell) - CANNOT PROGRESS while trading is suspended. Items 2, 4, 5, 6, 7, 9
wait for Checkpoint B; 10 (M39 corporate actions) and 11 (IBKR news) are
CLOSED - do not re-litigate. Full list: HANDOFF.md.

WHAT 14 AND 15 SEPTEMBER ESTABLISHED
⚠️ TRACK THE WORK IN THE OPERATOR'S NUMBERING (CE-030). One tracker, the
brief's numbers, status nowhere else.
⚠️ AN EMPTY JOIN IS EVIDENCE ABOUT THE JOIN (CE-029). Look the record up
directly before telling the operator it is missing.
⚠️ WRITE EACH FIGURE WITH ITS SOURCE OPEN (CE-031, CE-033, CE-067): slips in
every draft, and in the finalisation, were caught only by checking each
number against its tool output or source.
⚠️ A SUBSTITUTED VALUE HAS READERS (CE-032). Before calling a stand-in inert,
trace every reader; before saying what a setting controls, list its readers.
...
⚠️ A CONTROL THAT NEVER ACTED MAY NEVER HAVE BEEN WIRED (CE-043). Check the
wiring before calling it quiet.
⚠️ A WARNING THAT EXPLAINS A DISCREPANCY AWAY must name its evidence
(CE-066: TNE's and SEK's shortfalls were logged as "likely an adopted
position").
...
⚠️ BEFORE CITING AN "UNKNOWN" OR "NOT DETERMINED", SEARCH THE LATER SECTIONS
AND THE COMMITS (CE-036, CE-038).
⚠️ AN EVIDENCE RE-RUN USES THE ARGUMENTS THE SECTION WAS DRAFTED WITH, and a
tool whose output is hashed must print in a stable order (CE-067).
⚠️ ON A RESUME, READ `git log` AND THE TRACKER BEFORE SAYING WHERE THE WORK
STANDS (CE-035). Never answer a request to continue with no action. One
session per repository at a time.

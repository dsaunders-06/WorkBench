...
| Repository HEAD | ⚠️ **AHEAD by M159**, which is built and NOT deployed on purpose - see Deploy gap |
| Deploy gap | ⚠️ **M159 NOT DEPLOYED, DELIBERATELY.** `DEPLOYED` reads `16ce8e6` (M158), which is what is running. **Do NOT relaunch today**: the absorb query floor reaches back past every remembered fill, the most recent is PNI at `00:00:13Z`, and JHX's executions at `00:30Z` fall inside it - on a fresh process they fail all three guards at `oms.py:1487` and read as FOREIGN. Deploy at the close, launch TOMORROW, when today's executions have aged out of IBKR's same-day retention |
...
| Kill switch | ⚠️ **TRIPPED 31 Aug 10:30:15** - `Broker reconciliation mismatch`, JHX.AX tracked=1097 broker=378, logged **9 seconds into a 45-second market order**. The mismatch RESOLVED by 10:30:51 and has not recurred. **LEAVE IT TRIPPED**: at 10 of 10 positions no entry can happen anyway, exits are broker-side, and the JHX quarantine is unresolved. ⚠️ **The one condition that changes this: `POSITION UNPROTECTED`.** The switch blocks the app from placing ANY order including a protective repair. 20 of 20 legs are resting now |

...
- **Deployed:** M56+M51 (`3081fb7`), verified by hash against the build.
- **Repo:** clean, pushed, one branch (`master`), HEAD `f404dd9`.
...
1. **Watch tonight's session** — see the paste block below. Four things in this
   build have never met a moving market.
...
STATE
Deployed build M56+M51 (3081fb7), verified by hash. App is CLOSED and needs
launching before the open. Repo clean and pushed at f404dd9. Equity $101,363,
ten positions — AMAT, AMD, CRWD, CSCO, GS, JNJ, MS, UNP, VRTX, WFC — all
protected. One closed trade on record (CVS, −$482, −1.68R, stopped out).

WHAT HAS NEVER RUN IN A LIVE SESSION
This build carries four things that have only ever been exercised in tests or
against a paused account. Watching them is the point of tonight:

  1. The two evaluation sections in the daily report (M51). They will be
     generated for the first time at the close. Verify they appear and that the
     numbers match what an ad-hoc query of risk_decisions.csv says.
  2. The equity chart on a real time axis, seeded from equity_curve.csv (M56).
     It should open showing history rather than blank, and an overnight gap
     should render as a gap.
  3. Partial-fill absorption (M53). Only exercised if a stop fills in pieces —
  ...
  4. Refusal-on-broker-failure (M54). Only visible if Alpaca errors. A 500
...
Their ABSENCE is the signal, not their content.
...
One habit worth keeping: on 5–6 August, three of four defects introduced passed
...
broker. Read the files, not only the log lines you expect.

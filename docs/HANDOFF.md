...
| | |
|---|---|
| Deployed build | **M156 (`17f5ca9`)**, installed 28 August 21:35, SHA256 `B1B9E99D…8566`, signature Valid on the installed copy. Rollback: `C:\QuantAdvisoryTerminal.bak-9bd111a-20260828-2135`. ✅ **READ BACK 30 August 21:13 and again 21:23** — build line, ten positions adopted 10/10 with stops, 20 legs clean, and NO header-repair or quarantine lines, which is M150 and the 27 August clearance confirming themselves. The 48-hour watermark warning fired and `closed_trades.csv` was checked afterwards as it instructs: byte-identical. Carries item 44's `reference_price` (⚠️ **which M157 then found never reached the lot**), item 20's guard sweep, item 18's honest label, and items 13, 21, 40 closed. **Items 18 and 20 both CONFIRMED LIVE on 30 August** — the acknowledge line names the regime and says nothing was applied; `Advisory context for TNE.AX: 10 position(s), IS held` appeared and the model's note used the holding, quoting 3,051 shares and a −0.72% unrealised loss that both check out. ✅ **Deployed with `scripts/deploy.ps1`** — the first deploy to use it, which is item 68's actual remedy. Previous: M155 (Milestone C, items 22/31/59), M154 (item 67), M153, M152, M151, M150, M149, M148 |
| Repository HEAD | ⚠️ **AHEAD of the deployed build by M157.** `handoff_state.py` derives the gap; do not read a number from here |
| Deploy gap | ⚠️ **M157 IS BUILT BUT NOT DEPLOYED.** `DEPLOYED` reads `17f5ca9` (M156); HEAD is `9673bc0`. Derive this from `handoff_state.py`, never from here |
| Pushed | ⚠️ **74 COMMITS BEHIND `origin`.** The operator's standing hold from 27 August is still in force. ⚠️ **GitHub capacity RESET on 30 August**, so the billing wall — and the 20 failure emails in a week it caused — is gone. That removes the hold's supporting argument, **not the hold**: it is a standing operator instruction and only the operator lifts it. Do not infer it from the capacity news. A NAS copy covers the backup risk |
| Suite | **2,963 passed, 26 skipped** (2,989 collected). ruff, black, mypy src, bandit clean |

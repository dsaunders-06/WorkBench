...
| | |
|---|---|
| Deployed build | **M157 (`0167399`)**, installed 30 August 22:21, SHA256 `FD662F6C…9197`, signature Valid on the installed copy. Rollback: `C:\QuantAdvisoryTerminal.bak-17f5ca9-20260830-2221`. ✅ **READ BACK 30 August 22:22** — `Build: M157`, ten entry records, ten positions adopted 10/10 with stops, 20 legs clean, ten lots restored, no header-repair or quarantine lines. ⚠️ **And the number that mattered: `Excursion backfilled from daily bars on 10 of 10 restored lot(s); 0 had no bars`.** N of 0 would have meant the feature shipped onto a path nothing reaches, which is M151's shape. Verified further against yfinance independently of the app: all ten carry DISTINCT excursions, none equal to the entry price. ✅ Deployed with `scripts/deploy.ps1`, second deploy to use it. Previous: M156 (read back 30 Aug), M155, M154, M153, M152, M151, M150, M149, M148 |
| Repository HEAD | ⚠️ **AHEAD by M158 — DELIBERATELY.** M157 runs today's open; M158 deploys after the close so its read-back lands at a real bell. `handoff_state.py` derives the gap |
| Deploy gap | ⚠️ **M158 BUILT, NOT DEPLOYED — on purpose.** `DEPLOYED` reads `0167399` (M157), which is what is running today. Derive this from `handoff_state.py`, never from here |

...
| 6 | **Nothing is logged when the application stops.** Found 7 August when the process disappeared between two screenshots. The operator confirmed they had closed it, which is what makes the point: a deliberate, orderly shutdown wrote **nothing at all**, so it is indistinguishable in the log from the silent death that let M56a masquerade as a quiet session. A line at exit, plus a "previous run ended without one" note at startup, would separate them. Logging only, so freeze-safe and can land any time | Confirmed |

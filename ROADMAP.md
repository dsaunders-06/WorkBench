...
| 6 | **Nothing is logged when the application stops.** Found 7 August when the process disappeared between two screenshots: no error, no fault record, and no shutdown line - so an ordinary close and a silent death are indistinguishable in the log, which is the same blind spot that let M56a look like a quiet session. A single line at exit, and a "previous run ended without one" note at startup, would separate them. Logging only, so freeze-safe and can land any time | Observed |
